# -*- coding: utf-8 -*-
# Kuulemma
# Copyright (C) 2014, Fast Monkeys Oy
#
# This program is free software: you can redistribute it and/or modify
# it under the terms of the GNU Affero General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# This program is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU Affero General Public License for more details.
#
# You should have received a copy of the GNU Affero General Public License
# along with this program.  If not, see <http://www.gnu.org/licenses/>.

from datetime import date

# Main section
# ------------

title = 'Meilahden ja Pikku Huopalahden alueella tarkastellaan kouluverkkoa'

slug = 'meilahdenkoulut'

main_image = {
    'filename': 'images/meilahdenkoulut/main_image.jpg',
    'caption': (
        'Meilahden nykyinen kouluverkko. '
        '(Opetusvirasto / Päivi El Mahboul)'
    )
}

lead = '''


'''

body = '''

    <p>
    Meilahden ja Pikku Huopalahden alueella tarkastellaan syksyn aikana kouluverkkoa. Koulujen määrää
    ja tilojen tarvetta selvitetään. Tarkastelu perustuu opetuslautakunnan
    päätökseen ja kaupunginvaltuuston
    <a href="http://www.hel.fi/static/public/hela/Kaupunginhallitus/Suomi/Paatos/2015/Kanslia_2015-03-30_Khs_13_Pk/5A9C0AEB-D2FD-4026-B3E4-DC167594207A/Opetustoimen_palveluverkon_kehittamisen_tavoitteet.html">
    palveluverkkolinjauksiin</a>.
    </p>
    <p>
    Elokuussa 2016 peruskouluissa otetaan käyttöön uudet opetussuunnitelmat. 
    Niissä korostuu uudenlainen tapa oppia ja opettaa. Koulujen tiloja
    käytetään joustavammin ja monipuolisemmin. Uusien opetussuunnitelmien myötä
    myös opetettavien aineiden määrään ja valinnaisuuteen tulee muutoksia,
    jotka vaikuttavat koulujen toimintaan. Kouluverkkotarkastelulla pyritään
    ennakoimaan tulevia muutoksia ja tarjoamaan eri alueilla laadukkaat ja
    monipuoliset opetuspalvelut.
    </p>
    <p>
    Lapsimäärä Meilahden ala-asteen koulun lähistöllä kasvaa. Koulun tilat eivät tule riittämään kaikille alueella asuville oppilaille, kun taas Pikku Huopalahden ala-asteen koulussa on jonkin verran ylimääräistä tilaa.
    </p>
    <p>
    Koulupalvelujen kehittämisessä mietitään nyt eri vaihtoehtoja. Meilahden ja Pikku Huopalahden kouluverkkotarkastelun ehdotukset on tuotettu ideariihessä, jossa olivat mukana koulujen johtokunnat ja henkilöstön edustus. Ideariihen jälkeen koulut ovat kommentoineet ehdotuksia ja täydentäneet niitä omilla näkemyksillään.
    </p>
    <p>
    Esillä olevissa vaihtoehdoissa koulujen toimintaedellytyksiä tasataan aluemuutoksen ja opetuksen painotusten avulla.
    </p>
    <p class="lead">
    Kysymys:
    Mikä vaihtoehdoista 1-3 olisi sinusta paras kouluverkkoratkaisu Meilahden ja Pikku Huopalahden alueella? Voit myös esittää muuta ratkaisua.</p>
    </p>
    <p>
    Kommentoi ehdotuksia 25.10. saakka. Saatua palautetta hyödynnetään jatkosuunnittelussa.
    </p>
    <p>
    Meilahden ja Pikku Huopalahden alueen kouluverkosta järjestetään kaikille avoin asukastilaisuus tiistaina 20.10.2015 kello 18.00 -19.30 Meilahden yläasteen koulussa os. Kuusitie 12, Helsinki 27. Asukastilaisuuden jälkeen valmistuu opetusviraston esitys kouluverkon tarkistamisesta. Sitä voi kommentoida 15.11.2015 saakka. Tästä tiedotetaan vielä tarkemmin. 
    </p>
    <p>
    Lisätietoa opetusviraston
    <a href="http://www.hel.fi/www/opev/palveluverkko-fi/peruskoulut/">palveluverkkosivuilta</a>.   
    </p>
    <h4>Lisätietoja</h4>
    <p>
    aluepäällikkö Arjariitta Heikkinen, puh. 09 310 71765,
    <a href="mailto:etunimi.sukunimi@hel.fi">etunimi.sukunimi@hel.fi</a>
    </p>

'''  # noqa

opens_at = date(2015, 10, 19)

closes_at = date(2015, 10, 25)

published = True

area = {
    'type': 'Polygon',
    'coordinates': [
        [
            [
                24.91647720336914,
                60.20468677990855
            ],
            [
                24.915962219238278,
                60.204174981621726
            ],
            [
                24.917078018188477,
                60.20383377832825
            ],
            [
                24.91677761077881,
                60.20349257148627
            ],
            [
                24.917163848876953,
                60.20253290821839
            ],
            [
                24.91776466369629,
                60.2024902558661
            ],
            [
                24.917378425598145,
                60.19647571909273
            ],
            [
                24.922571182250977,
                60.19444929710485
            ],
            [
                24.92227077484131,
                60.19346803728698
            ],
            [
                24.9224853515625,
                60.193212051640224
            ],
            [
                24.923429489135742,
                60.192806736949755
            ],
            [
                24.924416542053223,
                60.191356095993605
            ],
            [
                24.92244243621826,
                60.19069475310575
            ],
            [
                24.9210262298584,
                60.18986272215973
            ],
            [
                24.91652011871338,
                60.18969204653849
            ],
            [
                24.91471767425537,
                60.189329357896995
            ],
            [
                24.913644790649414,
                60.188902660247386
            ],
            [
                24.912700653076172,
                60.188390615746805
            ],
            [
                24.910812377929688,
                60.18730249466484
            ],
            [
                24.90978240966797,
                60.18647037773795
            ],
            [
                24.908366203308105,
                60.18516881922682
            ],
            [
                24.90622043609619,
                60.185318181020975
            ],
            [
                24.882187843322754,
                60.1722144336769
            ],
            [
                24.868412017822266,
                60.17759315147788
            ],
            [
                24.88175868988037,
                60.191654762607804
            ],
            [
                24.8909854888916,
                60.19715827517058
            ],
            [
                24.891157150268555,
                60.20142392903852
            ],
            [
                24.883818626403805,
                60.20737358992917
            ],
            [
                24.884934425354004,
                60.2082904667964
            ],
            [
                24.889183044433594,
                60.208567657083265
            ],
            [
                24.889955520629883,
                60.208951455152
            ],
            [
                24.89922523498535,
                60.207864015627386
            ],
            [
                24.904160499572754,
                60.207991951560224
            ],
            [
                24.909439086914062,
                60.20846104571166
            ],
            [
                24.913344383239746,
                60.206030212425404
            ],
            [
                24.914245605468746,
                60.205859620869475
            ],
            [
                24.914588928222656,
                60.20521989463408
            ],
            [
                24.91647720336914,
                60.20468677990855
            ]
        ]
    ]
}

# Alternative 1
# -------------

alternative_1_title = 'Vaihtoehto 1'

alternative_1_main_image = {
    'filename': 'images/meilahdenkoulut/main_image.jpg',
    'caption': 'Meilahden nykyinen kouluverkko. '
               '(Opetusvirasto / Päivi El Mahboul)'
}

alternative_1_lead = '''

    Yhtenäinen peruskoulu:

'''

alternative_1_body = '''

<p>
Pikku Huopalahden ala-asteen koulu ja Meilahden yläasteen koulu yhdistyvät yhtenäiseksi peruskouluksi 1.8.2017 alkaen. Meilahden ala-asteen koulu säilyy nykyisellään.
</p>
<p>
Vaihtoehtoisesti joko:
</p>
<p><ul>
<li>Kiinan kielen opetus siirretään osittain tai kokonaan Meilahden ala-asteen koulusta Pikku Huopalahden ala-asteen kouluun.</li>
<li>Ruskeasuon alue siirretään Meilahden ala-asteen koulun oppilaaksiottoalueelta Pikku Huopalahden ala-asteen koulun oppilaaksiottoalueeseen.</li>
</ul></p>
<p>
Alueelle syntyy noin 680 oppilaan yhtenäinen peruskoulu, joka toimii kahdessa toimipisteessä. Koulu tarjoaa painotettua opetusta (kuvataide), johon haetaan soveltuvuuskokeella. Koulussa alkaa kokeiluna 1. luokalta lähtien varhennettu A-kielen (englanti) opetus.
</p>

'''

# Alternative 2
# -------------

alternative_2_title = 'Vaihtoehto 2'

alternative_2_main_image = {
    'filename': 'images/meilahdenkoulut/main_image.jpg',
    'caption': 'Meilahden nykyinen kouluverkko. '
               '(Opetusvirasto / Päivi El Mahboul)'
}

alternative_2_lead = '''

    Yhtenäinen peruskoulu:

'''

alternative_2_body = '''

<p>
Vanhan Ruskeasuon alue liitetään Pikku Huopalahden ala-asteen koulun oppilaaksiottoalueeseen 1.8.2017 alkaen. Koulun oppilasmäärä kasvaa vähitellen noin sadalla oppilaalla. Vastaavasti Meilahden ala-asteen koulun oppilasmäärä ei kasva. Pikku-Huopalahden ala-asteen koulussa alkaa kokeiluna 1. luokalta lähtien varhennettu A-kielen (englanti) opetus. Koulussa painotetaan kuvataiteen opetusta.
</p>

'''

# Alternative 3
# -------------

alternative_3_title = 'Vaihtoehto 3'

alternative_3_main_image = {
    'filename': 'images/meilahdenkoulut/main_image.jpg',
    'caption': 'Meilahden nykyinen kouluverkko. '
               '(Opetusvirasto / Päivi El Mahboul)'
}

alternative_3_lead = '''

    Meilahden yhteinen oppilaaksiottoalue:

'''

alternative_3_body = '''

<p>
Meilahden ja Pikku Huopalahden ala-asteen koulut muodostavat yhteisen oppilaaksiottoalueen.
</p>
<p>
Opetusviraston perusopetuslinjan aluepäällikkö päättää vuosittain, miten uudet oppilaat sijoitetaan kouluihin. Päätöksenteon pohjana ovat opetuslautakunnan määrittelemät
<a href="http://www.hel.fi/www/Helsinki/fi/paivahoito-ja-koulutus/perusopetus/oppilaaksiotto/">oppilaaksioton kriteerit</a>:
</p>
<p><ol type="a">
<li>Nuoremmalle samassa osoitteessa asuvalle sisarukselle osoitetaan sama koulu, jossa vanhempi sisarus on oppilaana päätöksentekohetkellä. Periaatetta ei sovelleta, jos vanhempi sisarus on koulun ylimmällä luokalla, hänet on otettu hakemuksen perusteella kouluun, tai koulu on osoitettu hänelle, koska hän on erityisoppilas.</li>
<li>Oppilaan koulumatka on mahdollisimman turvallinen ja lyhyt.</li>
</ol></p>

'''

# Hearing
# -------

hearing = {
    'slug': slug,
    'title': title,
    'main_image': main_image,
    'lead': lead,
    'body': body,
    'opens_at': opens_at,
    'closes_at': closes_at,
    'published': published,
    'area': area,
    'alternatives': [
        {
            'title': alternative_1_title,
            'main_image': alternative_1_main_image,
            'lead': alternative_1_lead,
            'body': alternative_1_body,
        },
        {
            'title': alternative_2_title,
            'main_image': alternative_2_main_image,
            'lead': alternative_2_lead,
            'body': alternative_2_body,
        },
        {
            'title': alternative_3_title,
            'main_image': alternative_3_main_image,
            'lead': alternative_3_lead,
            'body': alternative_3_body,
        },
    ]
}
